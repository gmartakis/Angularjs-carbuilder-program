...
class BuilderController extends Zend_Controller_Action {
	...
	public function init() {
		Zend_Layout::getMvcInstance() -> setLayout('builder');
		$contextSwitch = $this -> _helper -> getHelper('contextSwitch');
		$contextSwitch -> addActionContext('configure', 'json') -> initContext();
	...
	public function configureAction() {
		...
		if (null !== $this -> ColorSet) {
			foreach ($this->ColorSet as $Color => $Property) {
				$colors[] = array('color' => array('@attributes' => array('imageurl' => $Property['image']), 'colorname' => array('#text' => $Property['description']), 'rgbcolor' => array('#text' => $Property['rgb']), 'colorprice' => array('#text' => $Property['price']), 'colortrims' => array(), 'type' => array('#text' => $Property['type'])));
			}
		}
		//Trims for Car
		...
		if (null !== $this -> AccessorySet) {
			foreach ($this->AccessorySet as $Category => $Item) {
				foreach ($Item as $Accessory => $Property) {
					$accessories[] = array('accessory' => array('@attributes' => array('accessoryimagethumb' => $Property['image_thumb'], 'accessoryimagefull' => $Property['image_full']), 'accessoryname' => array('#text' => $Property['teaser']), 'sku' => array('#text' => $Property['sku']), 'accessorydesc' => array('#cdata-section' => $Property['description']), 'accessoryprice' => array('#text' => $Property['price'])));
				}
			}
		}
						...
						if (isset($this -> RiggingSet[$vehicleKey])) {
							...
							foreach ($riggingsByVehicle as $category) {
								foreach ($category as $item) {
									$riggings[$vehicleKey][] = array('category' => array('@attributes' => array('name' => $item['category']), 'rigging' => array('riggingname' => array('#cdata-section' => $item['name']), 'riggingtype' => array('#text' => $item['type']), 'riggingprice' => array('#text' => $item['price']))));
								}
							}
